...
for file_iter = 1:length(filename_list)
    ...
    for frame_iter = 1:numberOfPages
        ...
        if use_accelerate
            temp_resize_factor = 2; % use resize_factor = 2 to accelerate computation, or just use 10
            ...
            temp_img_for_HMRF = imresize(img_series(:, :, frame_iter), resize_factor);
            [HMRFseg, ~] = HMRFseg4img(temp_img_for_HMRF, imresize(nucleus_mask, resize_factor, "nearest"), nclust, 0.1, 10^(-8));
            bw_HMRF = HMRFseg.img_class>=seg_point;
            bw_local = bw_HMRF; 
        end
    
        CDcenter(:, :, frame_iter) = getCDcenter(double(uint16(img_series(:, :, frame_iter))), bw_local, resize_factor, 0);
        CDinterface(:, :, frame_iter) = getCDinterface(temp_img, bw_local, 0);
        [~, temp_labels] = getCDboundary(temp_img, bw_local, CDcenter(:, :, frame_iter), CDinterface(:, :, frame_iter), 0);
    
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % add area and enrichment filter
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [temp_labels, stats] = filter_condensates_Area_PC(temp_labels, temp_img, min_pixel_num, min_partition_coefficient, resize_factor);
