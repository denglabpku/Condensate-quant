...
if with_RNA
    output_path = fullfile(filepath, 'filter_result_with_RNA_average_boundary');
    mkdir(output_path)

    filename_list = readlines(fullfile(filepath, 'Cell_with_RNA.txt'));
    filename_list = strrep(filename_list, "'", "");
else
    output_path = fullfile(filepath, 'filter_result_without_RNA_average_boundary');
    mkdir(output_path)
...
for file_iter = 1:length(filename_list)
    ...
    diff_z_layer = spots_405(3)-spots_488(3);
    disp(['Z gap between channel 405 and 488 is ', num2str(diff_z_layer, '%.2f')])
    z_layer_405 = round(spots_405(3));
    z_layer_488 = round(spots_488(3));
    z_layer_used = max(min(z_layer_405, z_layer_488), layer_select(1)):min(max(z_layer_405, z_layer_488), sizeZ-1);
    ...
    img_series_max(:, :, :, 3) = mean(channel_560(:, :, z_layer_used), 3);
...
for c_iter = 3:4 % OCT4 and BRD4 channel
    ...
    for frame_iter = 1:numberOfPages
        ...
        seg_point = seg_points(c_iter-2);% Threshold for condensate class, above this will be considered as condensates
        ...
        [temp_labels, stats] = filter_condensates_Area_PC(temp_labels, img_processed_bicubic_roi(:, :, frame_iter), min_pixel_num, min_partition_coefficient(c_iter-2), resize_factor);
    ...
    dist2center = zeros(2, numberOfPages);
    condensate_radius = zeros(2, numberOfPages);
    equiv_radius = zeros(2, numberOfPages);
    boundary2center = zeros(2, numberOfPages);
    dist2boundary = zeros(2, numberOfPages);
    dist2centroid = zeros(2, numberOfPages);
    boundary2centroid = zeros(2, numberOfPages);
...
save(fullfile(output_path, [filename_base, '.mat']), "condensate_result", "dist_dna2rna", '-append');
...
colormap gray
hold on
visboundaries(condensate_result(3).CDmask, 'Color', 'g', 'LineWidth', 1);
...
title(['OCT4 (layer:', num2str(z_layer_used), ')'],'FontSize',11,'FontWeight','bold')
hold off
...
title(['BRD4 (layer:', num2str(z_layer_used), ')'],'FontSize',11,'FontWeight','bold')
hold off
...
filepath_withRNA = fullfile(filepath, 'filter_result_with_RNA_average_boundary');
filepath_withoutRNA = fullfile(filepath, 'filter_result_without_RNA_average_boundary');
...
for file_iter = 1:length(filename_withRNA_list)
    ...
    load(fullfile(filepath_withRNA, filename), "foci_result", "condensate_result", "channel_labels", "dist_dna2rna");
...
for file_iter = 1:length(filename_woRNA_list)
    ...
    load(fullfile(filepath_withoutRNA, filename), "foci_result", "condensate_result", "channel_labels", "dist_dna2rna");
    %generate random localizations
    rand_index = [(1 + (roi_width-2)*rand)*resize_factor, (1 + (roi_width-2)*rand)*resize_factor];
...
for file_iter = 1:length(filename_withRNA_list)
    ...
    dist_summary_Prom_RNA(file_iter).filename = filename;
    dist_summary_Prom_RNA(file_iter).spots_405_3D = foci_result(1).spots_3D.*[px, py, pz];
    dist_summary_Prom_RNA(file_iter).spots_488_3D = foci_result(2).spots_3D.*[px, py, pz];
    dist_summary_Prom_RNA(file_iter).distance = norm(dist_summary_Prom_RNA(file_iter).spots_405_3D-dist_summary_Prom_RNA(file_iter).spots_488_3D);
...
for file_iter = 1:length(filename_withRNA_list)
    ...
    dist_summary_withRNA(file_iter).dna2oct4 = condensate_result(3).dist2boundary(1);
    dist_summary_withRNA(file_iter).dna2brd4 = condensate_result(4).dist2boundary(1);
    dist_summary_withRNA(file_iter).rna2oct4 = condensate_result(3).dist2boundary(2);
    dist_summary_withRNA(file_iter).rna2brd4 = condensate_result(4).dist2boundary(2);
    dist_summary_withRNA(file_iter).rnd2oct4 = condensate_result(3).rand2boundary;
    dist_summary_withRNA(file_iter).rnd2brd4 = condensate_result(4).rand2boundary;
    dist_summary_withRNA(file_iter).dna2rna = dist_dna2rna;
end
...
for file_iter = 1:length(filename_woRNA_list)
    ...
    dist_summary_woRNA(file_iter).dna2oct4 = condensate_result(3).dist2boundary(1);
    dist_summary_woRNA(file_iter).dna2brd4 = condensate_result(4).dist2boundary(1);
    dist_summary_woRNA(file_iter).rna2oct4 = condensate_result(3).dist2boundary(2);
    dist_summary_woRNA(file_iter).rna2brd4 = condensate_result(4).dist2boundary(2);
    dist_summary_woRNA(file_iter).rnd2oct4 = condensate_result(3).rand2boundary;
    dist_summary_woRNA(file_iter).rnd2brd4 = condensate_result(4).rand2boundary;
    dist_summary_woRNA(file_iter).dna2rna = dist_dna2rna;
end

%% calculate distance from DNA(promoter)/RNA sites to boundary, centroids and condensate radius
...
for file_iter = 1:length(filename_withRNA_list)
    ...
    dist_summary_withRNA(file_iter).filename = filename;
    dist_summary_withRNA(file_iter).dna2oct4 = condensate_result(3).dist2centroid(1);
    dist_summary_withRNA(file_iter).dna2brd4 = condensate_result(4).dist2centroid(1);
    dist_summary_withRNA(file_iter).rna2oct4 = condensate_result(3).dist2centroid(2);
    dist_summary_withRNA(file_iter).rna2brd4 = condensate_result(4).dist2centroid(2);
    dist_summary_withRNA(file_iter).rnd2oct4 = condensate_result(3).rand2centroid;
    dist_summary_withRNA(file_iter).rnd2brd4 = condensate_result(4).rand2centroid;
    dist_summary_withRNA(file_iter).dna2rna = dist_dna2rna;
end
...
for file_iter = 1:length(filename_woRNA_list)
    ...
    dist_summary_woRNA(file_iter).filename = filename;
    dist_summary_woRNA(file_iter).dna2oct4 = condensate_result(3).dist2centroid(1);
    dist_summary_woRNA(file_iter).dna2brd4 = condensate_result(4).dist2centroid(1);
    dist_summary_woRNA(file_iter).rna2oct4 = condensate_result(3).dist2centroid(2);
    dist_summary_woRNA(file_iter).rna2brd4 = condensate_result(4).dist2centroid(2);
    dist_summary_woRNA(file_iter).rnd2oct4 = condensate_result(3).rand2centroid;
    dist_summary_woRNA(file_iter).rnd2brd4 = condensate_result(4).rand2centroid;
    dist_summary_woRNA(file_iter).dna2rna = dist_dna2rna;
end


%% Summary distance to centroids and condensate radius

filepath = fullfile(script_dir, 'Promoter_RNA_OCT4_BRD4'); dna = 'Promoter';
% filepath = fullfile(script_dir, 'SCR_RNA_OCT4_BRD4'); dna = 'SCR';

filepath_withRNA = fullfile(filepath, 'filter_result_with_RNA_average_boundary');
filepath_withoutRNA = fullfile(filepath, 'filter_result_without_RNA_average_boundary');


% dna2oct4_centoids, dna2oct4_radius, dna2brd4_centoids, dna2brd4_radius, rna2oct4_centoids, rna2oct4_radius, rna2brd4_centoids, rna2brd4_radius, 
filename_withRNA_list = dir(fullfile(filepath_withRNA, '*.mat'));
dnarna2oct4brd4_centroid_radius = zeros(length(filename_withRNA_list), 8);
rand2oct4brd4_centroid_radius_rna = zeros(length(filename_withRNA_list), 4);
for file_iter = 1:length(filename_withRNA_list)
    filename = filename_withRNA_list(file_iter).name;
    load(fullfile(filepath_withRNA, filename), "foci_result", "condensate_result", "channel_labels", "dist_dna2rna");

    dnarna2oct4brd4_centroid_radius(file_iter, :) = [condensate_result(3).dist2centroid(1), condensate_result(3).boundary2centroid(1), ...
                                                     condensate_result(4).dist2centroid(1), condensate_result(4).boundary2centroid(1), ...
                                                     condensate_result(3).dist2centroid(2), condensate_result(3).boundary2centroid(2), ...
                                                     condensate_result(4).dist2centroid(2), condensate_result(4).boundary2centroid(2)];
    % dnarna2oct4brd4_centroid_radius(file_iter, :) = [condensate_result(3).dist2center(1), condensate_result(3).condensate_radius(1), ...
    %                                                  condensate_result(4).dist2center(1), condensate_result(4).condensate_radius(1), ...
    %                                                  condensate_result(3).dist2center(2), condensate_result(3).condensate_radius(2), ...
    %                                                  condensate_result(4).dist2center(2), condensate_result(4).condensate_radius(2)];
    rand2oct4brd4_centroid_radius_rna(file_iter, :) = [condensate_result(3).rand2centroid, condensate_result(3).randradius, ...
                                                       condensate_result(4).rand2centroid, condensate_result(4).randradius];
end

% dna2oct4_centoids, dna2oct4_radius, dna2brd4_centoids, dna2brd4_radius
filename_woRNA_list = dir(fullfile(filepath_withoutRNA, '*.mat'));
dna2oct4brd4_centroid_radius = zeros(length(filename_woRNA_list), 4); 
rand2oct4brd4_centroid_radius = zeros(length(filename_woRNA_list), 4);
for file_iter = 1:length(filename_woRNA_list)
    filename = filename_woRNA_list(file_iter).name;
    load(fullfile(filepath_withoutRNA, filename), "foci_result", "condensate_result", "channel_labels", "dist_dna2rna");

    dna2oct4brd4_centroid_radius(file_iter, :) = [condensate_result(3).dist2centroid(1), condensate_result(3).boundary2centroid(1), ...
                                                     condensate_result(4).dist2centroid(1), condensate_result(4).boundary2centroid(1)];
    % dna2oct4brd4_centroid_radius(file_iter, :) = [condensate_result(3).dist2center(1), condensate_result(3).condensate_radius(1), ...
    %                                                  condensate_result(4).dist2center(1), condensate_result(4).condensate_radius(1)];
    rand2oct4brd4_centroid_radius(file_iter, :) = [condensate_result(3).rand2centroid, condensate_result(3).randradius, ...
                                                       condensate_result(4).rand2centroid, condensate_result(4).randradius];
end

rand2oct4brd4_centroid_radius = [rand2oct4brd4_centroid_radius; rand2oct4brd4_centroid_radius_rna];
...
nexttile
plot_scatter_panel(dnarna2oct4brd4_centroid_radius(:,1), ...
                   dnarna2oct4brd4_centroid_radius(:,2), ...
                   c_blue, ms, alpha, range_x_limit, range_y_limit)
title([dna ' (ON) – OCT4'])

nexttile
plot_scatter_panel(dnarna2oct4brd4_centroid_radius(:,5), ...
                   dnarna2oct4brd4_centroid_radius(:,6), ...
                   c_red, ms, alpha, range_x_limit, range_y_limit)
title('RNA (ON) – OCT4')

nexttile
plot_scatter_panel(dna2oct4brd4_centroid_radius(:,1), ...
                   dna2oct4brd4_centroid_radius(:,2), ...
                   c_purple, ms, alpha, range_x_limit, range_y_limit)
title([dna ' (OFF) – OCT4'])

nexttile
plot_scatter_panel(rand2oct4brd4_centroid_radius(:,1), ...
                   rand2oct4brd4_centroid_radius(:,2), ...
...
nexttile
plot_scatter_panel(dnarna2oct4brd4_centroid_radius(:,3), ...
                   dnarna2oct4brd4_centroid_radius(:,4), ...
                   c_blue, ms, alpha, range_x_limit, range_y_limit)
title([dna ' (ON) – BRD4'])

nexttile
plot_scatter_panel(dnarna2oct4brd4_centroid_radius(:,7), ...
                   dnarna2oct4brd4_centroid_radius(:,8), ...
                   c_red, ms, alpha, range_x_limit, range_y_limit)
title('RNA (ON) – BRD4')

nexttile
plot_scatter_panel(dna2oct4brd4_centroid_radius(:,3), ...
                   dna2oct4brd4_centroid_radius(:,4), ...
                   c_purple, ms, alpha, range_x_limit, range_y_limit)
title([dna ' (OFF) – BRD4'])

nexttile
plot_scatter_panel(rand2oct4brd4_centroid_radius(:,3), ...
...
xlabel(t,'Foci to condensate centroid (nm)','FontSize',13,'FontWeight','bold')
...
% -------- DNA (ON)
nexttile
plot_density_contour(dnarna2oct4brd4_centroid_radius(:,1), ...
                     dnarna2oct4brd4_centroid_radius(:,2), ...
...
% -------- RNA (ON)
nexttile
plot_density_contour(dnarna2oct4brd4_centroid_radius(:,5), ...
                     dnarna2oct4brd4_centroid_radius(:,6), ...
...
% -------- DNA (OFF)
nexttile
plot_density_contour(dna2oct4brd4_centroid_radius(:,1), ...
                     dna2oct4brd4_centroid_radius(:,2), ...
...
% -------- Random
nexttile
plot_density_contour(rand2oct4brd4_centroid_radius(:,1), ...
                     rand2oct4brd4_centroid_radius(:,2), ...
...
% -------- DNA (ON)
nexttile
plot_density_contour(dnarna2oct4brd4_centroid_radius(:,3), ...
                     dnarna2oct4brd4_centroid_radius(:,4), ...
                     gridpts, xg, yg, c_blue, range_x_limit, range_y_limit)
title([dna ' (ON) – BRD4'])
...
% -------- RNA (ON)
nexttile
plot_density_contour(dnarna2oct4brd4_centroid_radius(:,7), ...
                     dnarna2oct4brd4_centroid_radius(:,8), ...
                     gridpts, xg, yg, c_red, range_x_limit, range_y_limit)
title('RNA (ON) – BRD4')
...
% -------- DNA (OFF)
nexttile
plot_density_contour(dna2oct4brd4_centroid_radius(:,3), ...
                     dna2oct4brd4_centroid_radius(:,4), ...
                     gridpts, xg, yg, c_purple, range_x_limit, range_y_limit)
title([dna ' (OFF) – BRD4'])
...
s1 = scatter(dnarna2oct4brd4_centroid_radius(:,1), ...
             dnarna2oct4brd4_centroid_radius(:,2), ...
             ms, c_blue, 'filled');
s1.MarkerFaceAlpha = alpha;

s2 = scatter(dnarna2oct4brd4_centroid_radius(:,5), ...
             dnarna2oct4brd4_centroid_radius(:,6), ...
             ms, c_red, 'filled');
s2.MarkerFaceAlpha = alpha;

s3 = scatter(dna2oct4brd4_centroid_radius(:,1), ...
             dna2oct4brd4_centroid_radius(:,2), ...
             ms, c_black, 'filled');
s3.MarkerFaceAlpha = alpha;
plot([0 range_y_limit],[0 range_y_limit],'--','Color',[0.5 0.5 0.5],'LineWidth',1);

daspect([1 1 1])
xlim([0 range_x_limit]); ylim([0 range_y_limit])

xlabel('Distance to OCT4 condensate centroid (nm)')
ylabel('OCT4 condensate radius (nm)')
...
s1 = scatter(dnarna2oct4brd4_centroid_radius(:,3), ...
             dnarna2oct4brd4_centroid_radius(:,4), ...
             ms, c_blue, 'filled');
s1.MarkerFaceAlpha = alpha;

s2 = scatter(dnarna2oct4brd4_centroid_radius(:,7), ...
             dnarna2oct4brd4_centroid_radius(:,8), ...
             ms, c_red, 'filled');
s2.MarkerFaceAlpha = alpha;

s3 = scatter(dna2oct4brd4_centroid_radius(:,3), ...
             dna2oct4brd4_centroid_radius(:,4), ...
             ms, c_black, 'filled');
s3.MarkerFaceAlpha = alpha;
plot([0 range_y_limit],[0 range_y_limit],'--','Color',[0.5 0.5 0.5],'LineWidth',1);
...
xlabel('Distance to BRD4 condensate centroid (nm)')
ylabel('BRD4 condensate radius (nm)')
