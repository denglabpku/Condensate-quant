...
class training_class():
    ...
    def __init__(self, params_dict):
        ...
        self.gap_x = 115
        self.gap_t = 115
        self.masking_ratio = 0.05
        self.neighbor_radius_xy = 5
        self.neighbor_radius_t = 1
        self.lr = 0.00001
        self.b1 = 0.5
        self.b2 = 0.999
        self.GPU = '0'
    ...
    def initialize_network(self):
        """
        Initialize U-Net 3D network, which is the main network architecture of 3D-N2V
        ...
        """
        unet_type = '2DUNet' if self.patch_t == 1 else '3DUNet'
        denoise_generator = Network_3D_Unet(UNet_type = unet_type,
                                            in_channels=1,
                                            out_channels=1,
                                            f_maps=self.fmap,
                                            final_sigmoid=True)
    ...
    def get_gap_t(self):
        ...
        print('s_num -->', s_num)
        self.gap_t = math.floor((self.whole_t - self.patch_t) / (s_num - 1))
        print('gap_t -->', self.gap_t)
        ...
        for im_name in list(os.walk(self.datasets_path, topdown=False))[-1][-1]:
            ...
            for x in range(0, int((self.whole_y - self.patch_y + self.gap_y) / self.gap_y)):
                for y in range(0, int((self.whole_x - self.patch_x + self.gap_x) / self.gap_x)):
                    for z in range(0, int((self.whole_t - self.patch_t + self.gap_t) / self.gap_t)):
                        ...
                        init_s = self.gap_t * z
                        end_s = self.gap_t * z + self.patch_t
    ...
    def save_yaml_train(self):
        ...
        para = {'n_epochs': 0, 'datasets_path': 0, 'overlap_factor': 0,
                ...
                'masking_ratio': 0, 'neighbor_radius_xy': 0, 'neighbot_radius_t': 0,
        ...
        para["masking_ratio"] = self.masking_ratio
        para["neighbor_radius_xy"] = self.neighbor_radius_xy
        para["neighbot_radius_t"] = self.neighbor_radius_t
        para["lr"] = self.lr
        para["b1"] = self.b1
        para["b2"] = self.b2
        para["fmap"] = self.fmap
    ...
    def train(self):
        ...
        for epoch in range(0, self.n_epochs):
            train_data = trainset(self.name_list, self.coordinate_list, self.noise_im_all, self.stack_index, self.masking_ratio, self.neighbor_radius_xy, self.neighbor_radius_t)
            trainloader = DataLoader(train_data, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers)
            for iteration, (input, target, mask) in enumerate(trainloader):
                ...
                L1_loss = L1_pixelwise(fake_B*mask, real_B*mask)
                L2_loss = L2_pixelwise(fake_B*mask, real_B*mask)
    ...
    def test(self, train_epoch, train_iteration):
        ...
        for iteration, (noise_patch, single_coordinate) in enumerate(testloader):
            ...
            postprocess_turn = real_A.shape[0]
            if postprocess_turn > 1:
                output_image = np.squeeze(fake_B.cpu().detach().numpy(), axis=1)
                raw_image = np.squeeze(real_A.cpu().detach().numpy(), axis=1)
            elif postprocess_turn == 1:
                output_image = np.squeeze(fake_B.cpu().detach().numpy(), axis=(0, 1))
                raw_image = np.squeeze(real_A.cpu().detach().numpy(), axis=(0, 1))
