...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='cr', num_groups=8,
        ...
        if isinstance(f_maps, int):
            ...
            f_maps = create_feature_maps(f_maps, number_of_fmaps=4) # [16, 32, 64, 128]
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='cr', num_groups=8,
        ...
        for i, out_feature_num in enumerate(f_maps):
            if i == 0:
                ...
                encoder = Encoder(f_maps[i - 1], out_feature_num, pool_kernel_size=(1, 2, 2), basic_module=DoubleConv, 
                                  conv_layer_order=layer_order, num_groups=num_groups)
            encoders.append(encoder)
